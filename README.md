...
Такое измерение было проведено три раза и усреднено для каждого хеша. Среднеквадратичное отклонение рассчитывалось по формуле: $\sum {\frac{\sqrt{(x_n^2 - x_{mean}^2)}}{n}}$, где $x_n$ -- длина n-той цепочки, а x -- средняя длина цепочки.
...
### Выводы
Благодаря грамотному подходу к оптимизациям, исходная функция поэтапно была ускорена в 2.7 раз, причем различными методами:
