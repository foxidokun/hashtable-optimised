...
Вспомним, что кеш линия имеет размер 64 байта, а значит можно оптимизировать локальность строк,
используя структуру
```c++
...
Ожидаемо, после такой оптимизации функция рассчета хеша практически скрылась с графа вызовов.
![v3flame.png](images%2Fscreenshots%2Fv3flame.png)

Соптимизируем функцию strcmp, написав ее на ассемблере с учетом известной нам длины ключа (32 байта):
```asm
asm_strcmp_noinline:
...
Поскольку мы уже соптимизировали до предела хеш функцию и strcmp, остается только оптимизировать саму
...
соптимизированны максимально эффективными способами. Дальнейшее увеличение размера хештаблицы смысла не имеет, так как 
...
заметить отсутствие strcmp в списке функций perf'a, что означает, что она была заинлайнена. С учетом того, что 
компилятор и так выбирал avx2-реализацию strcmp, это по сути эквивалентно проделанной мной оптимизации.
